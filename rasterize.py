...
from colors import make_palette # noqa
from tile_utils import tiles_from_csv # noqa
...
def main(args):
    ...
    if os.path.isfile(args.features):
        ...
        geojsons = [
            os.path.join(args.features, fn)
            for fn in os.listdir(args.features)
            if fn.lower().endswith(".geojson") and os.path.isfile(os.path.join(args.features, fn))
        ]
        if not geojsons:
            raise FileNotFoundError(f"Папка {args.features} не содержит .geojson файлов")
        for file in geojsons:
            with open(file, "r", encoding="utf-8") as f:
                fc = json.load(f)
                rasterize(args, fc)
    else: 
        raise FileNotFoundError(f"{args.features} не найден или не является файлом/папкой")

    


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Создает бинарные маски объектов.")
    parser.add_argument("--zoom", type=int, required=True, help="Уровень масштабирования.")
    parser.add_argument("--size", type=int, default=256, help="Размер изображения маски в пиксилях.")
    parser.add_argument("--features", type=str, required=True, help="Путь к папке/файлу с .geojson")
    parser.add_argument("--tiles", type=str, required=True, help="Путь к .csv файлу с тайлами.")
    parser.add_argument("--out", type=str, default='data/masks',help="Путь к папке куда будут сохрянятся маски.")
    parser.add_argument("--dataset", type=str, default='config/dataset-building.toml', help="Путь к файлу с конфигурацией датасета.")
